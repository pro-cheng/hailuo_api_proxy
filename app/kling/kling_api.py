...
class BaseGen:
    def __init__(self, cookie: str) -> None:
        ...
        self.info_url = f"{self.base_url}api/user/profile"
    ...
    def get_account_info(self) -> dict:
        info_req = self.session.get(self.info_url)
        info_data = info_req.json()
        assert info_data.get("status") == 200
        return info_data
...
class VideoGen(BaseGen):
    ...
    def submit_video_task(
        ...
        if image_path or image_url:
            ...
            model_type = "m2v_img2video_hq" if is_high_quality else "m2v_img2video"
            payload = {
                "arguments": [
                ...
                "inputs": [{"inputType": "URL", "url": image_payload_url, "name": "input"}],
                "type": model_type,
            }
            
            if tail_image_path or tail_image_url:
                tail_image_payload_url = self.image_uploader(tail_image_path) if tail_image_path else tail_image_url
                for arg in payload["arguments"]:
                    if arg["name"] == "tail_image_enabled":
                        arg["value"] = "true"
                payload["inputs"].append({"inputType": "URL", "url": tail_image_payload_url, "name": "tail_image"})

        else:
            model_type = "m2v_txt2video_hq" if is_high_quality else "m2v_txt2video"
            payload = {
                "arguments": [
                    {"name": "prompt", "value": prompt},
                    {"name": "negative_prompt", "value": ""},
        ...
        response = self.session.post(self.submit_url, json=payload)
        if not response.ok:
            print(response.text)
            raise Exception(f"Error response {str(response)}")
        response_body = response.json()
    ...
    def get_video_task_info(self, request_id: str) -> dict:
        image_data, status = self.fetch_metadata(request_id)
        return image_data,status

    def get_video_status(self, request_id: str, auto_extend: bool = False) -> list:
        start_wait = time.time()
        print("Waiting for results... will take 2mins to 5mins")
        while True:
            if int(time.time() - start_wait) > 1200:
            ...
            if status == TaskStatus.PENDING:
                ...
                if auto_extend:
                    ...
                    return self.extend_video(request_id)
                return result
    ...
    def save_video(
        ...
        image_path: Optional[str] = None,
        image_url: Optional[str] = None,
        tail_image_path: Optional[str] = None,  # 添加 tail_image_path 参数
        tail_image_url: Optional[str] = None,   # 添加 tail_image_url 参数
        is_high_quality: bool = False,
...
# aspect_ratio_list = ["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"]
class ImageGen(BaseGen):
        ...
        if image_path or image_url:
            ...
            payload = {
                "arguments": [
                    ...
                    {"name": "style", "value": "默认"},
                    {"name": "aspect_ratio", "value": aspect_ratio},
                    {"name": "imageCount", "value": image_count},
    ...
    def get_image_task_info(self, request_id: str) -> dict:
        image_data, status = self.fetch_metadata(request_id)
        return image_data,status
    
    def get_image_task_status(self, request_id: str) -> list:
        start_wait = time.time()
...
def main():
    ...
    parser.add_argument(
        "--tail-image-path",
        help="Tail image file path if you want to use a tail image",
        type=str,
        default="",
    )
    parser.add_argument(
        "--tail-image-url",
        help="Tail image URL if you want to use a tail image",
        type=str,
        default="",
    )
    ...
    if args.type == "image":
        ...
        video_generator.save_video(
            ...
            output_dir=args.output_dir,
            image_path=args.I,
            tail_image_path=args.tail_image_path,
